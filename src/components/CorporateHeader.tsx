  ...
  return <header className="fixed top-0 w-full bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 z-50 border-b shadow-sm">
      <nav className="container mx-auto px-3 sm:px-4 lg:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8">
          <Link to="/" className="text-sm font-medium text-foreground/80 hover:text-primary transition-all duration-200 relative group">
            ​Home
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Button onClick={() => scrollToSection("kontakt")} className="bg-primary hover:bg-primary/90 shadow-md hover:shadow-lg transition-all duration-200 hidden sm:inline-flex text-sm lg:text-base px-3 lg:px-4 h-9 lg:h-10">
            Kontakt aufnehmen
          </Button>
          
          {/* Mobile Menu Button */}
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="lg:hidden p-2 hover:bg-accent rounded-md transition-colors" aria-label="Menu">
      ...
      {isMenuOpen && <div className="lg:hidden absolute top-full left-0 w-full bg-background backdrop-blur-xl border-b shadow-lg">
          <div className="container mx-auto px-3 sm:px-4 py-4 flex flex-col gap-3">
            ...
            <button onClick={() => scrollToSection("hautkrebs")} className="text-base font-medium text-foreground/80 hover:text-primary transition-colors text-left py-2 border-b border-border/50">
              Hautkrebs
            </button>
            <button onClick={() => scrollToSection("angebot")} className="text-base font-medium text-foreground/80 hover:text-primary transition-colors text-left py-2 border-b border-border/50">
              Unser Angebot
            </button>
            <button onClick={() => scrollToSection("kontakt")} className="text-base font-medium text-foreground/80 hover:text-primary transition-colors text-left py-2 border-b border-border/50">
              Kontakt
            </button>
            <Button onClick={() => scrollToSection("kontakt")} className="w-full mt-2">
              Kontakt aufnehmen
            </Button>
          </div>
        </div>}
